...
MAPIT = function(X, y, Z = NULL,C = NULL,threshold = NULL,cores = 1){
  ### Run the analysis using the Normal/Z-test ###
  
  ### Run Model 1 by Default ###
  if(is.null(Z)==TRUE&&is.null(C)==TRUE){
    vc.mod = MAPIT1_Normal(X,y,cores=cores)
  }
  ### Test to Run Model 2 ###
  if(is.null(Z)==FALSE&&is.null(C)==TRUE){
    vc.mod = MAPIT2_Normal(X,y,Z,cores=cores)
  }
  ### Test to Run Model 3 ###
  if(is.null(Z)==TRUE&&is.null(C)==FALSE){
    vc.mod = MAPIT3_Normal(X,y,C,cores=cores)
  }
  ### Test to Run Model 4 ###
  if(is.null(Z)==FALSE&&is.null(C)==FALSE){
    vc.mod = MAPIT4_Normal(X,y,Z,C,cores=cores)
  }
  ...
  if(is.null(threshold)==FALSE){
    ...
    ### Run Model 1 by Default ###
    if(is.null(Z)==TRUE&&is.null(C)==TRUE){
      vc.mod = MAPIT1_Davies(X,y,ind,cores=cores)
    }
    ### Test to Run Model 2 ###
    if(is.null(Z)==FALSE&&is.null(C)==TRUE){
      vc.mod = MAPIT2_Davies(X,y,Z,ind,cores=cores)
    }
    ### Test to Run Model 2 ###
    if(is.null(Z)==TRUE&&is.null(C)==FALSE){
      vc.mod = MAPIT3_Davies(X,y,C,ind,cores=cores)
    }
    ### Test to Run Model 2 ###
    if(is.null(Z)==FALSE&&is.null(C)==FALSE){
      vc.mod = MAPIT4_Davies(X,y,Z,C,ind,cores=cores)
    }
