...
MAPIT = function(X, y, W = NULL,C = NULL,hybrid = TRUE,threshold = 0.05,test = NULL,k = NULL,cores = 1){
  ...
  if(hybrid==TRUE){
    ...
    if(is.null(W)==FALSE&&is.null(C)==TRUE){
      vc.mod = MAPIT2_Normal(X,y,W,cores=cores)
    }
    ### Test to Run Model 3 ###
    ...
    if(is.null(W)==FALSE&&is.null(C)==FALSE){
      vc.mod = MAPIT4_Normal(X,y,W,C,cores=cores)
    }
    ...
    ### Test to Run Model 2 ###
    if(is.null(W)==FALSE&&is.null(C)==TRUE){
      vc.mod = MAPIT2_Davies(X,y,W,ind,cores=cores)
    }
    ### Test to Run Model 2 ###
    if(is.null(W)==TRUE&&is.null(C)==FALSE){
    ...
    ### Test to Run Model 2 ###
    if(is.null(W)==FALSE&&is.null(C)==FALSE){
      vc.mod = MAPIT4_Davies(X,y,W,C,ind,cores=cores)
    }
