...
// [[Rcpp::export]]
List MAPIT1_Normal(mat X,vec y){
    int i;
...
// [[Rcpp::export]]
List MAPIT2_Normal(mat X,vec y,mat Z){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
...
// [[Rcpp::export]]
List MAPIT3_Normal(mat X,vec y,mat C){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
...
// [[Rcpp::export]]
List MAPIT4_Normal(mat X,vec y,mat Z,mat C){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
    const int q = Z.n_rows;
...
// [[Rcpp::export]]
List MAPIT1_Davies(mat X,vec y,const vec ind){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
...
// [[Rcpp::export]]
List MAPIT2_Davies(mat X,vec y,mat Z,const vec ind){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
    const int q = Z.n_rows;
    ...
    for(i=0; i<p; i++){
        ...
        if(std::find(ind.begin(), ind.end(), i+1) != ind.end()){
            ...
            //Compute K and G covariance matrices
            mat K = (GSM*p-trans(X.row(i))*X.row(i))/(p-1); //Create the linear kernel
            mat G = K; //Create the marginal epistasis kernel
            G.each_row() %= X.row(i);
            G.each_col() %= trans(X.row(i));
...
// [[Rcpp::export]]
List MAPIT3_Davies(mat X,vec y,mat C,const vec ind){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
...
// [[Rcpp::export]]
List MAPIT4_Davies(mat X,vec y,mat Z,mat C,const vec ind){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
    const int q = Z.n_rows;
...
// [[Rcpp::export]]
List MAPIT_Davies_Approx(mat X,vec y){
    int i;
    const int n = X.n_cols;
    const int p = X.n_rows;
    ...
    for(i=0; i<p; i++){
        ...
        ///Compute K and G covariance matrices
        mat G = (GSM*p-trans(X.row(i))*X.row(i))/(p-1); //Create the Marginal Epistatic Kernel
        G.each_row() %= X.row(i);
        G.each_col() %= trans(X.row(i));
